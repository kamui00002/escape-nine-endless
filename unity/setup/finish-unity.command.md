...
- 実行順は docs/unity-migration-plan.md §8 の改訂順を優先: 0 → 1検証 → 2 UI → 4 juice → 6a Steam体験版(前倒し) → 5 ローグライク → 3 収益化(後ろへ) → 6b Android → 7。
- コスト規律 (同 §8 Tier 表): AI不要の作業(bootstrap/テスト/ビルド)はシェルで、機械的修正は ultracode なしで行い、ultracode は大きな設計/実装/検証フェーズのみ。1 Phase = 1 ワークフロー。各 Phase の間で結果を報告し、私の確認を待つ。
