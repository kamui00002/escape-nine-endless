...
- **予算スケール**: 大規模フェーズは先頭に `+200k`〜`+800k` 等の予算指定を付けると、ultracode が探索・検証をその範囲でスケールする。
...
### 3.5 クレジット管理（2026-07-01 追記 — 従量課金中は必読）

プラン付属枠を超えると extra usage クレジットの従量消費になる（= トークンは実費）。ultracode は並列エージェントを大量に走らせるため消費が速い。ルール:

1. **Tier 0 を先に**: `bootstrap.sh` / EditModeテスト / 実機ビルドは AI 不要。まずシェルで済ませる。
2. **機械的作業に ultracode を使わない**: コンパイルエラー修正・小結線は通常の Claude Code で（Tier 1）。
3. **ultracode は予算指定必須**: 例 `+200k /finish-unity 2`。上限なしで回さない。
4. **フェーズ単位で起動**（`/finish-unity <N>`）。マスター全自走はクレジット残高に余裕がある時だけ。
5. **`/usage` で残高を随時確認**。実行順は `docs/unity-migration-plan.md` §8（改訂順: 0→1→2→4→**6a Steam**→5→**3後ろ**→6b→7）に従う。
