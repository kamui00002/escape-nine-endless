...
| ナイト | 盾ガード（次の衝突を1回無効化） | 2回 | 有料¥240 |

- 10階層ごとにスキル使用回数がリセットされる
- 値の正本は `Models/Character.swift` / `Utilities/Constants.swift`

---

## ゲームバランス

### BPM曲線（べき乗: BPM = 70 + 130 × ((floor−1)/99)^1.4）
...
- 計算式の正本は `Models/Floor.swift` の `calculateBPM(for:)`
...
- Floor 36+: 自然Hard
- プレイヤー選択でさらに1段調整
- 10の倍数階（bossFloorInterval=10）はボス階: プレイヤーのAI選択に関わらず常にBoss AI（追跡率95%、bossAIChaseChance=0.95）
