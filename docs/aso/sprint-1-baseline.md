...
| 計測開始日 | `YYYY-MM-DD` |
| 計測終了日 | `YYYY-MM-DD` |
| 対象アプリバージョン | v1.0.0 (TestFlight + App Store) |
| プラットフォーム | iOS のみ |
| 地域 | 全世界 |
| 採取者 | (記入者名) |
...
| イベント | 7 日間カウント | 備考 |
|---|---|---|
| `eg_game_started` | _未取得_ | 母数 |
| `eg_floor_cleared` (floor==1) | _未取得_ | 1 階クリア数 |
| `eg_floor_cleared` (全 floor) | _未取得_ | クリア総数 |
| `eg_game_over_shown` | _未取得_ | Game Over 表示数 |
| `eg_retry_tapped` | _未取得_ | リトライ数 |
| `eg_home_tapped` | _未取得_ | ホーム戻り数 |

### 算出 KPI

| KPI | 実測値 | 目標値 (v1.1 後) | 改善幅 |
|---|---|---|---|
| 1 階離脱率 | _未取得_ | 30% 以下 | _未算出_ |
| 平均到達階層 | _未取得_ | (会議録に記載なし) | — |
| Game Over → リトライ率 | _未取得_ | (Sprint 1 既知の改善対象) | — |
| 平均セッション秒数 | _未取得_ | (60 秒問題分析用) | — |
| 平均ニアミス距離 | _未取得_ | (惜しさメーター検証用) | — |
| Day 1 リテンション | _未取得_ | +10pp | _未算出_ |
| Day 7 リテンション | _未取得_ | (参考、目標値未設定) | — |
