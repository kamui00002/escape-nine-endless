...
作成日: 2026-07-03
ステータス: **設計提案（実装未着手）** — 本書は `docs/unity-migration-plan.md` §4 のPhase 5スコープを具体化する設計書。コード変更は含まない。
...
- UI側は次のパターンを1ターン前にテレグラフ表示する（`GameController.OnBossPatternChanged` イベントを新設し、Phase 4 で整備済みの `FxKit`/`BeatPulse` 系の演出プリミティブを流用して実装できる）。
...
- 新規UI: `RelicDraftScreen` / `RouteChoiceScreen`（5c） / `MetaShopScreen`（5b+）。`ScreenBase` / `UIFactory` / Phase 4 の `FxKit`（レアリティ別グロー等）を再利用する。
...
- UI: `RelicDraftScreen`（**1/2/3キーで選択可能。Steam体験版のキーボード操作を前提にした必須要件**）
...
- 分岐ルート（`RouteChoice`、`RouteChoiceScreen`、デイリーチャレンジ中の無効化ルール）
- ボスパターン3種 + テレグラフUI + `OnBossPatternChanged`
...
8. **ボス「威圧」パターンとマス消失特殊ルールの視覚的混同**: 41階以降の恒久消失マスと、ボス限定の一時マーク（③威圧）が同じ盤面上でプレイヤーに区別できるUI表現になっているか。本書は仕様止まりで、具体的な見た目（色・アイコン差別化）までは踏み込んでいない。
