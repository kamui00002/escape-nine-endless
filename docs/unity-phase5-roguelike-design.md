...
| 8 | 灯火の指輪 | Common（上限1） | 霧の視界半径 +1マス（階層21+で有効） | LateGame | Tier1 |
...
| `ResolveTurn` の衝突分岐（`isCollision`） | 更新可能スキル（透明化/盾）を先に消費し、1ラン限りのレリックチャージ（`Relics.ReviveCharges`/`GenericShieldCharges`）を最後の砦にする（旧: レリック優先 → #12 を取ると取らない方がマシになる逆転があり修正。`Relics.None` 時の透明化/盾挙動は不変） |
...
## 9. 未決事項 — ✅ 2026-07-04 確定（オーナー一任により以下で確定）

> 1=ラン限り+スターターパーク1枠 / 2=デイリー中は無効化(v1) / 3=弱キャラ底上げ目標・100階クリア率10%警告ライン維持 /
> 4=**AIEngine は修正しない**（Swift 1:1 パリティ維持、レリック側で対処） / 5=残光の IAP 接続なし /
> 6=毎階層ドラフト（`RelicConfig.DraftInterval` 定数でチューニング可） / 7=Tier2 レリックは実機定性検証のみ /
> 8=解消済み / 9=v1 はテキスト+発光で出荷、アイコンはキャラ一新の出来を見て判断。
